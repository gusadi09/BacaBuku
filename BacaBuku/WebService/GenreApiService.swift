...
class GenreApiService: ObservableObject {
    let objectWillChange = ObservableObjectPublisher()
    
    @Published var dataTotal: GenList? {
        willSet {
    ...
    init() {
        guard let url = URL(string: "https://cabaca.id:8443/api/v2/cabaca/_table/genre") else {
        ...
        URLSession(configuration: .default).dataTask(with: urlRequest) { (data, response, error) in
            ...
            let result = try? JSONDecoder().decode(GenList.self, from: data)
