...
class PenulisApiService: ObservableObject {
  
  let objectWillChange = ObservableObjectPublisher()
  
  
  @Published var dataTotal: PenulisResponse? {
    willSet {
  ...
  init(id: String) {
    self.id = id
    guard let url = URL(string: "https://cabaca.id:8443/api/v2/writer/detail/\(id)") else {
    ...
    URLSession(configuration: .default).dataTask(with: urlRequest) { (data, response, error) in
      ...
      let result = try? JSONDecoder().decode(PenulisResponse.self, from: data)
