...
class BukuDetailApiService: ObservableObject {
  
  let objectWillChange = ObservableObjectPublisher()
  
  
  @Published var dataTotal: BukuDetailResponse? {
    willSet {
  ...
  init(id: String) {
    self.id = id
    guard let url = URL(string: "https://cabaca.id:8443/api/v2/book/detail/\(id)") else {
    ...
    URLSession(configuration: .default).dataTask(with: urlRequest) { (data, response, error) in
      ...
      let result = try? JSONDecoder().decode(BukuDetailResponse.self, from: data)
